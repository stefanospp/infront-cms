/**
 * Theorium Admin Theme
 *
 * Injects into admin pages via afterAuth middleware:
 * - White-label: replaces all SonicJs branding with Theorium
...
export const WHITE_LABEL: [string, string][] = [
  // Page titles
  ['SonicJS AI Admin', 'Theorium CMS'],
  ['SonicJS AI', 'Theorium CMS'],
  ['SonicJS', 'Theorium'],
  ['Sonic JS', 'Theorium'],

  // Dashboard text
  ['Welcome to your Theorium CMS admin dashboard', 'Welcome to the Theorium content management system'],
  ['A modern headless CMS powered by AI', 'Private Science & Maths Tutoring — Content Management'],

  // Login page
  ['Welcome Back', 'Theorium CMS'],
  ['Sign in to your account to continue', 'Sign in to manage your content'],
];

// Replace the SonicJs SVG logo + version badge with Theorium text logo.
// Uses regex because the SVG is a large inline blob that can't be matched with simple strings.
const THEORIUM_LOGO = '<span style="font-size:20px;font-weight:700;letter-spacing:-0.5px;color:#465FFF;">Theorium</span> <span style="font-size:11px;color:#94a3b8;font-weight:500;">CMS</span>';
...
export function whiteLabel(html: string): string {
  ...
  // Replace the SVG logo (viewBox="380 1300 2250 400") + version badge with Theorium text
  html = html.replace(
